	...
	*/
	...
	function chargeCard($loginid, $trankey, $ccnum, $ccexpmonth, $ccexpyear, $ccver, $live, $amount, $tax, $shipping, $desc, $billinginfo, $email, $phone, $shippinginfo) {
		...
		if ( AUTHORIZE_NET_ACCOUNT_TYPE == 'development' ) {
		  // zb 060409 -- not sure why there are the two development urls.  test.authorize.net seems to work for the dev account credentials we have
		...
		if ( AUTHORIZE_NET_ACCOUNT_TYPE == 'development' ) {
		  ...
		  // zb 060409 not sure if the line below is used... I think it stays commented out
		...
		/*
		 // zb 060409 -- these came with the component, but shouldn't be used unless we're on godaddy (or other shared host?)
