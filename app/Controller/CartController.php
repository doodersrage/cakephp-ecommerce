...
class CartController extends AppController {
	...
	private function calcPrices(){
		...
		if(isset($userCart)){
			foreach($userCart as $idx => $val){
				...
				if(!empty($product['Product']['shipIncQty']) && !empty($product['Product']['shipIncCost'])){
					...
					$shipArr['total'] = $shipArr['total'] + $shipArr[$idx]['selShipCost'];
				}
				...
				$prices[$idx] = $prices[$idx] * $dimVal * $val['qty'];
				$subTotal += $prices[$idx];
			}
		...
		return array($prices, $subTotal, (isset($shipArr) ? $shipArr : ''));
	}
			...
			if(!empty($this->request->data['email']) && !empty($this->request->data['firstName']) && !empty($this->request->data['lastName']) && !empty($this->request->data['company']) && !empty($this->request->data['telephone']) && !empty($this->request->data['address']) && !empty($this->request->data['city']) && !empty($this->request->data['state']) && !empty($this->request->data['postalCode'])){
				...
				if(!empty($this->request->data['shipEmail']) && !empty($this->request->data['shipFirstName']) && !empty($this->request->data['shipLastName']) && !empty($this->request->data['shipCompany']) && !empty($this->request->data['shipTelephone']) && !empty($this->request->data['shipAddress']) && !empty($this->request->data['shipCity']) && !empty($this->request->data['shipState']) && !empty($this->request->data['shipPostalCode'])){
					...
					if(!empty($this->request->data['nameOnCard']) && !empty($this->request->data['cardNumber']) && !empty($this->request->data['cardExpYear']) && !empty($this->request->data['cardExpMonth']) && !empty($this->request->data['cardCVV'])){
						...
						$response = $this->AuthorizeNet->chargeCard('953bEXWr58pa', '6q57We2bJ6hC9u33', $this->request->data['cardNumber'], $this->request->data['cardExpMonth'], $this->request->data['cardExpYear'], $this->request->data['cardCVV'], true, $orderTotal, 0, (!empty($prices[2]['total']) ? $prices[2]['total'] : 0), 'CGS Online Order', $billinginfo, $this->request->data['email'], $this->request->data['telephone'], $shippinginfo); 
						...
						if($response[1] == 1){
							...
							return $this->redirect(array('/cart/confirm/?o='.$orderId));
						} else {
							switch($response[1]){
								case 2:
				...
				'class' => 'alert-warning'
								...
								case 3:
								default:
									$this->Session->setFlash(__('warning','We are sorry, but there appears to have been an error in processing your payment.'), 'alert', array(
				'plugin' => 'BoostCake',
				'class' => 'alert-warning'
			));
								break;
...
//									$this->Order->clear();
//		
//									$this->set('warning','Your payment information has been held for review. Please contact CGS for further direction.');
//								break;
							}
						}
